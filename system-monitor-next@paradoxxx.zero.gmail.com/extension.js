...
const Battery = class SystemMonitor_Battery extends ElementBase {
    constructor(extension) {
        ...
        this.handler_id = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 1, this._poll_quickSettings.bind(this));
        log('handler_id = ' + this.handler_id);
    ...
    _poll_quickSettings() {
        ...
        if (typeof(this._proxy) === 'undefined') {
            if (typeof(Main.panel.statusArea.quickSettings._system) === 'undefined') {
                return GLib.SOURCE_CONTINUE;
            }
            if (typeof(Main.panel.statusArea.quickSettings._system._systemItem._powerToggle._proxy) === 'undefined') {
                return GLib.SOURCE_CONTINUE;
            } else {
                // set this._proxy and bind update_battery()
                this._proxy = Main.panel.statusArea.quickSettings._system._systemItem._powerToggle._proxy;
                this.powerSigID = this._proxy.connect('g-properties-changed', this.update_battery.bind(this));
                this.handler_id = undefined;
                return GLib.SOURCE_REMOVE;
            }
        }
    }
